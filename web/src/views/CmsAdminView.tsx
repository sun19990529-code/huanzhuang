...
export const CmsAdminView: React.FC<CmsAdminViewProps> = ({
  ...
  const handleConfirmResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`用户【${resetPassTargetUser.nickname}】登录密码已成功重置为: ${newPasswordVal.trim()}`);
  ...
  const handleConfirmBroadcast = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`全员活动积分已成功发放给 ${res.count} 位用户！`);
